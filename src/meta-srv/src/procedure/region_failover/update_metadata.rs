...
use common_meta::kv_backend::txn::{TxnOp, TxnRequest};
...
impl UpdateRegionMetadata {
    ...
    async fn update_metadata(
        ...
        let update_region_placement_txn =
            self.update_region_placement_txn(ctx, failed_region).await?;

        let update_table_route_txn = self.update_table_route_txn(ctx, failed_region).await?;
        unsafe {
            txn_req.append_with_uncheck(update_region_placement_txn);
            txn_req.append_with_uncheck(update_table_route_txn);
        }

        ctx.selector_ctx.kv_store.txn(txn_req.into()).await?;
    ...
    async fn update_region_placement_txn(
        &self,
        ctx: &RegionFailoverContext,
        failed_region: &RegionIdent,
    ) -> Result<TxnRequest> {
    ...
    async fn update_table_route_txn(
        &self,
        ctx: &RegionFailoverContext,
        failed_region: &RegionIdent,
    ) -> Result<TxnRequest> {
        ...
        Ok(TxnRequest {
            compare: vec![],
            success: vec![TxnOp::Put(key.to_string().into_bytes(), value.into())],
            failure: vec![],
        })
    }
}
